...
if [[ $(uname) == "Darwin" ]]; then
    ...
    echo "=== Installed successfully!  IMPORTANT: For changes to take effect, please close and reopen the terminal window, or run:  exec \$SHELL"
    ...
    if [[ "$ID" == "ubuntu" || "$ID" == "debian" || "$ID_LIKE" == "ubuntu" || "$ID_LIKE" == "debian" || "$ID_LIKE" == "ubuntu debian" ]]; then
        ...
        if [[ "$RET" == "0" ]]; then
          ...
          echo -e "\033[33mWe recommend removing this package, as it creates huge log files if a camera is used in UVC mode (webcam)\033[0m"
